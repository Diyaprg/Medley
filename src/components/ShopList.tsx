...
const ShopList = () => {
  return (
    <div className="w-full px-6 py-10 bg-gray-100">
      ...
      <div className="flex gap-5 overflow-x-auto scrollbar-hide pb-4">
        {shop.map((shop) => (
          <div
            key={shop.id}
            className="min-w-[260px] h-[300px] flex-shrink-0 bg-white rounded-2xl shadow-md hover:shadow-xl transition transform hover:-translate-y-1 flex flex-col"
          >
            {/* Card Header */}
    ...
    <User className="w-5 h-5 text-green-700" /> {/* Dark Green owner icon */}
    {shop.owner}
  </p>

  <p className="text-base font-medium text-gray-800 mb-1 flex items-center gap-2">
    <Phone className="w-5 h-5 text-blue-700" /> {/* Dark Blue phone icon */}
    {shop.phone}
  </p>
